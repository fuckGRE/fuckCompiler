 ...
 */
public class TypeChecker extends CompilerVisitor {
    ...
    public void check(AST ast) throws SemanticException {
        ...
        for (DefinedFunction func : ast.definedFunctions()) {
            ...
            checkParamterType(func);
            check(func.body());
        }
        if (errorHandler.errorOccured()) {
            throw new SemanticException("Complier error");
        }
    }

    private void checkReturnType(DefinedFunction func) {
        if (isInvalidReturnType(func.returnType())) {
            error(func.location(), "return invalid type: " + func.returnType());
        }
    }

    private void checkParamterType(DefinedFunction func) {
    ...
    public Void visit(ReturnNode node) {
        ...
        if (currentFunction.isVoid()) {
            ...
            if (node.expr() == null) {
                error(node, "mission return value");
                return null;
            }
    ...
    public Void visit(OpAssignNode node) {
        ...
        if (!mustBeInteger(node.lhs(), node.operator()))
            return null;
        if (!mustBeInteger(node.rhs(), node.operator()))
            return null;
        Type left = integralPromotion(node.lhs().type());
        Type right = integralPromotion(node.rhs().type());
        Type opType = usualArithmeticConversion(left, right);
        if (!opType.isCompatible(left) && !isSafeIntegerCast(node.rhs(), opType)) {
            warn(node, "incompatible implicit cast from " + opType + " to " + left);
        }
    ...
    private ExprNode forcePointerType(ExprNode master, ExprNode slave) {
        if (master.type().isCompatible(slave.type())) {
            ...
            warn(slave, "incompatible implict cast from " + slave.type() + " to " + master.type());
    ...
    private void expectsScalarLHS(UnaryArithmeticOpNode node) {
        ...
        if (node.expr().type().isInteger()) {
            ...
            throw new Error("must no happen");
        }
    }
